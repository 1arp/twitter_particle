  ...
  pJS.fn.particle.prototype.draw = function() {
    ...
    switch (p.shape) {
      case "circle-text" :
          ...
          if(p.x == pJS.interactivity.mouse.click_pos_x && p.y == pJS.interactivity.mouse.click_pos_y){
            alert('this is a test')
          }
          
          pJS.canvas.ctx.fillText('hello', p.x , p.y + 10)
          break;
      ...
      if (dist_mouse <= pJS.interactivity.modes.bubble.distance) {
        if (ratio >= 0 && pJS.interactivity.status == "mousemove") {
          ...
          if (pJS.interactivity.modes.bubble.size != pJS.particles.size.value) {
           ...
           // my_code
             var mouse_click_dx = p.x - pJS.interactivity.mouse.click_pos_x,
              mouse_click_dy = p.y - pJS.interactivity.mouse.click_pos_y,
              dist_mouse_click = Math.sqrt(mouse_click_dx * mouse_click_dx + mouse_click_dy * mouse_click_dy),
              ratio = 1 - dist_mouse_click / pJS.interactivity.modes.bubble.distance;
              
              if (dist_mouse_click <= pJS.interactivity.modes.bubble.distance ){
                if(ratio >= 0 && pJS.interactivity.status == "mousemove"){
...
window.particlesJS.load = function(tag_id, config_json, callback) {
  /* load json config */
  var params = JSON.parse(config_json);
